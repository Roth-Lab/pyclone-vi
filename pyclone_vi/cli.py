...
@click.option(
    ...
    help='''Allele count density in the PyClone model.'''
    '''Use beta-binomial for high coverage sequencing.'''
    '''Default binomial.'''
)
@click.option(
    '-g', '--num-grid-points',
...
@click.option(
    ...
    default=1.0,
    type=float,
    help='''Exponent of entries in the annealing ladder.'''
    '''Default is 1.0.'''
)
@click.option(
...
@click.option(
    ...
    '''Higher values will produce more clusters.'''
...
@click.option(
    ...
    default=200,
    type=float,
    help='''Precision for Beta-Binomial density.'''
    '''Has no effect when using Binomial.'''
    '''Default is 200.'''
)
@click.option(
    '--print-freq',
